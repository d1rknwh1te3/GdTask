...
public partial class Test : Node
{
	...
	private async GdTaskVoid Run()
	{
		...
		var runResult = await RunWithResult();
		...
		await GdTask.Delay(TimeSpan.FromSeconds(3), cancellationToken: cts.Token);
		await cts.CancelAsync();
		...
		await GdTask.NextFrame(cts.Token);
	...
	private async GdTaskVoid RunPause()
	{
		...
		var result = await RunWithResultPause();
		...
		await GdTask.Delay(TimeSpan.FromSeconds(3), PlayerLoopTiming.PauseProcess, cts.Token);
		await cts.CancelAsync();
		...
		await GdTask.NextFrame(PlayerLoopTiming.PauseProcess, cts.Token);
